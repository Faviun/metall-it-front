...
interface CartContextProps {
  ...
  clearCart: () => void;
  selectSupplierForItem: (productId: number, supplierName: string | undefined) => void; 
}
...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addToCart = (product: MetalProduct) => {
    ...
    if (existingItem) {
      ...
      setItems([...items, { ...product, quantity: quantityToAdd, availableSuppliers: product.suppliers, selectedSupplierName: undefined }]);
    }
  };
  ...
  const selectSupplierForItem = (productId: number, supplierName: string | undefined) => {
    setItems(prevItems =>
      prevItems.map(item =>
        item.id === productId ? { ...item, selectedSupplierName: supplierName } : item
      )
    );
  };
  ...
  return (
    <CartContext.Provider value={{ items, addToCart, removeFromCart, updateQuantity, clearCart, selectSupplierForItem }}>
      {children}
